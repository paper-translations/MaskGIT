...
\usepackage[pagebackref,breaklinks,colorlinks,linkcolor=blue,hyperfootnotes=false]{hyperref}
...
\begin{center}
    ...
    \vspace{-6mm}
    \captionof{figure}{\textbf{MaskGIT 在图像合成和处理任务上的生成示例}。我们展示了 MaskGIT 是一个灵活的模型，可以在以下任务上生成高质量的样本：(a)类别条件合成，(b)类别条件图像处理（例如将边界框内的选定对象替换为给定类别中的对象），以及(c)图像外推。这里显示的示例在三列中的分辨率分别为 512$\times$512、512$\times$512 和 512$\times$2560。放大查看详细信息。} 
    \vspace{1mm}
	\label{fig:teaser}
\end{center}%
}]
\footnotetext{$^\ast$ 目前隶属于微软 Azure AI。}
