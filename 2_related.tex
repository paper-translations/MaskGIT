...
\subsection{具有双向Transformer的掩码建模}
...
特别是，BERT~\cite{Devlin19bert}引入了掩码语言建模（MLM）任务用于语言表示学习。BERT~\cite{Devlin19bert}中使用的双向自注意力允许使用来自两个方向的上下文来预测MLM中的掩码token。
在视觉中，BERT~\cite{Devlin19bert}中的掩码建模已扩展到图像表示学习~\cite{he2021mae, Bao2022Beit}，其中图像被量化为离散token。然而，很少有工作成功地将相同的掩码建模应用于图像生成~\cite{zhang2021ufcbert}，因为使用双向注意进行自回归解码很困难。
据我们所知，本文提供了第一个证据，证明了掩码建模对于在常见ImageNet基准上的图像生成的有效性。
