...
%    & &  \\
    ...
    \vspace{-5pt}
    \caption{与 ImageNet 256$\times$256 和 512$\times$512 上最先进的生成模型的定量比较。
    %
    \footnotesize{``\# steps” 指生成样本所需的神经网络运行次数。$^\ast$ 表示我们用与我们相同的架构和设置训练的模型；$^\square$ 表示从先前出版物中获取的值；$^\dagger$ 基于 PyTorch 实现估计~\cite{pytorch2020vqvae2}。}
    }
