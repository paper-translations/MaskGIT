\newcolumntype{N}{@{}m{0pt}@{}}

\begin{table*}[h]
\small
    \centering
    {\small
    \begin{tabular}{lc ccc ccc ccc cc}
    \toprule
     {Model} & 
    
     & {FID}  $\downarrow$ & {IS}  $\uparrow$ & 
     &  {Prec}  $\uparrow$ & {Rec}   $\uparrow$ &
     
     &{\# params} & {\# steps} &
     
     & \multicolumn{2}{c}{{CAS $\times 100$} $\uparrow$}  
     \\
     \cline{12-13}
    &&  && & &&& & && {Top-1 (76.6)} & {Top-5 (93.1)} \\[-7pt]
    \textbf{ImageNet 256$\times$256} && &&& &&& &&& &
    \\[-2pt]
    \midrule
    

     
    DCTransformer~\cite{nash2021generating} $^\square$ &
    & 36.51 & n/a &
    & 0.36 & \textbf{0.67} &
    & 738M & $>$1024 &   & &
     \\ 
    
    BigGAN-deep~\cite{biggan}  & %, 1.0 trunc. &
    & 6.95 & \bfseries{198.2} &
    & \textbf{0.87} & 0.28 &
    & 160M & 1 & & 43.99 &67.89 \\
    \\
    
    % Improved DDPM~\cite{nichol2021improved} &
    % & 31.5 &   &
    % & ?0.70 & ?0.62 &
    % &  & &
    % & 270M & 250 \\
    Improved DDPM~\cite{nichol2021improved}$^\square$  &
    & 12.26 & n/a  &
    & 0.70 & 0.62 &
     & 280M & 250  &  &  & \\
    
    ADM~\cite{dhariwal2021diffusion}$^\square$   & 
    & 10.94 & 101.0  &
    & 0.69 & 0.63 &
    & 554M & 250 &  &  & \\
    
    % ADM-G, 1.0 guid. & 
    % & 4.59 & 186.7  &
    % & ?0.69 & ?0.63 &
    % &  &  &
    % & 554M & 250  %554M + 54M classfier
    % \\

    
    VQVAE-2~\cite{Razavi19vqvae2}$^\square$ &
    & 31.11 & $\sim$45 &
    & 0.36 & 0.57 &
    & 13.5B$^{\blackdiamond}$ & 5120 & & 54.83 & 77.59 \\
   % \midrule
   % \bfseries{\model  Baseline} && 18.98 & 192.52 && ? & ? && ?? & ?? \\
    \midrule
    VQ-GAN~\cite{Esser21vqgan}$^\square$ &
    
    & 15.78 & 78.3 &
    & n/a & n/a &
    & 1.4B & 256 &  &  &
    \\
 
    VQ-GAN$^\ast$ &
    & 18.65 & 80.4 &
    & 0.78 & 0.26 &
    & 227M & 256 & 
    & 53.10 $^\dagger$ & 76.18 $^\dagger$
     \\
 &
    & & &
    & & &
    &  & & 
    & 47.50 & 68.90
     \\
    % VQ-GAN, 0.05 RS &
    % & 1.4B +  & 5120 &
    % & 5.20 & 380.3 &
    % % & ? & ? &
    % &  & \\
%     \bfseries{\model -Linear} &
    
%     &  &  &
%     &  &  &
%   & 227M  & 16 &
%      & &
   
    \bfseries{\model  (Ours)} &
    
    & \bfseries{\bestfid} & \bestis &
    & 0.80 & 0.51 &
   & 227M  & 8 &
   & \bfseries{63.14}$^\dagger$ & \bfseries{84.45}$^\dagger$ 
    \\
    &&  & &
    & & &
   & & &
   & 58.20 & 79.65
    \\
    % 227M + 54M
    % \bfseries{Ours, 0.05 RS} &
    % & +  &  &
    % & 5.90 & 152.9 &
    % % & ? & ? &
    % &  &  \\

   
    % Validation &
    % &- & - &
    % & 1.62 & 234.0  &
    % % & ? & ? &
    % &  73.09 &  91.47 \\
    \bottomrule
    \\[-4pt]
    \textbf{ImageNet 512$\times$512} && &&& &&& &&& &\\[-2pt]
    \midrule
    
     BigGAN-deep~\cite{biggan} & & 8.43 & \bfseries{232.5} & & \textbf{0.88} & 0.29 &
     &160M&1&&44.02&68.22
     \\
     
     ADM~\cite{dhariwal2021diffusion}$^\square$ & & 23.24 & 58.06 &&0.73& \textbf{0.60} & 
     &559M&250&
     && \\
    %  ADM, 1.0 guid. & 7.72 & & 0.87 & 0.42  \\
     \midrule
     VQGAN$^\ast$ & & 26.52 & 66.8 &
     & 0.73 & 0.31 &
      & 227M &1024 &
      &51.29&74.24 \\
     \bfseries{\model  (Ours)} & & \bfseries{\bestfidhighres} & 156 & &  0.78 & 0.50 & & 227M &12&
     &63.43&84.79 \\
     % \bfseries{Ours}, 0.5 RS & ? & ? & &  \\
    \bottomrule
    
    \end{tabular}
    }
    \vspace{-5pt}
    \caption{ImageNet 256$\times$256和512$\times$512上与最先进的生成模型的定量比较。
    %
    \footnotesize{``\# steps”是指生成样本所需的神经网络运行次数。$^\ast$表示我们用与我们相同的架构和设置训练的模型；$^\square$表示从先前的出版物中获取的值；$^\blackdiamond$基于pytorch实现估计~\cite{pytorch2020vqvae2}。CAS分数是在收敛的模型上计算的，该模型训练了300个epoch，其中$^\dagger$表示用强数据增强Randaugment训练的模型~\cite{cubuk2020randaugment}。}
    }
    \label{tab:maintable}
    % On average 10-15ms per step

\end{table*}